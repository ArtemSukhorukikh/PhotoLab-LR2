<?php
    ...
    $userPage = new user();
    $userPage->getData((isset($_GET["id"]) ? $_GET["id"] : $_COOKIE['userEmail']),(isset($_GET["id"]) ? 'id' : 'email'));
    $userPage->getUsersPostsData($userPage->email);
    $flagMyPage = $userPage->email == $_COOKIE['userEmail'];
;?>
        ...
        <div class="row">
            ...
            ?>
                    ...
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item"><?=(empty($userPage->userPosts[$i]->middleRating) ? 'Оценок еще нет' : $userPage->userPosts[$i]->middleRating)?></li>
                        <li class="list-group-item"><?=(empty($userPage->userPosts[$i]->countRatings) ? 'Оценок еще нет' : $userPage->userPosts[$i]->countRatings)?></li>
                    ...
                    <div class="card-body">
                        ...
                        if (!$flagMyPage) {
                            echo '
                                ...
                                <a type="button" href="#" class="btn btn-primary my-2">Поставить оценку</a>
                                </form>
